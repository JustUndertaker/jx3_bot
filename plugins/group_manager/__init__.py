...
async def _(bot: Bot):
    ...
    for group in group_list:
        ...
        await group_init(group_id=group_id,group_name=group_name)
        # 用户注册
...
get_group_list=on_regex(pattern=r"(^状态$)|(^运行状态$)",rule=check_event(group_list_event),permission=SUPERUSER, priority=2, block=True)
...
set_group_status=on_regex(pattern=group_status_regex,rule=check_event(group_list_event),permission=SUPERUSER, priority=2, block=True)
...
change_all=on_regex(pattern=change_all_regex,rule=check_event(group_list_event),permission=SUPERUSER, priority=2, block=True)
